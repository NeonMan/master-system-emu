...
///Writes to VDP control port.
static void vdp_control_write() {
...
///Reads of VDP control port.
static void vdp_control_read() {
    vdp.control_index = 0;
    z80_data = vdp.status;
}

///Reads of VDP data port.
static void vdp_data_read() {
    vdp.control_index = 0;
    z80_data = vdp.buffer;
...
///Writes to VDP data port.
static void vdp_data_write() {
    vdp.control_index = 0;
    vdp.buffer = z80_data;
    switch (vdp.control_mode) {
    case VDP_CTRL_REGISTER:
...
static void vdp_io() {
    ...
    case ((1 << 6) | 1): //Odd address H counter (read) [0x7F]
        if (z80_n_rd) //Only allow reads
            return;
        assert_warning((z80_address & 0x00FF) == VDP_PORT_H_COUNTER);
    ...
    case ((1 << 6) | 0): //Even address V counter (read) [0x7E]
        if (z80_n_rd) //Only allow reads
            return;
        assert_warning((z80_address & 0x00FF) == VDP_PORT_V_COUNTER);
        vdp.ioreq_done = 1; //<-- Set internal IO to prevent spurious IO
    ...
    case ((1 << 7) | 1): //Odd address is control port
        if (!z80_n_rd) { //Control port read
            assert_warning((z80_address & 0x00FF) == VDP_PORT_CONTROL);
            vdp_control_read();
        }
    ...
    case ((1 << 7) | 0): //Even address is data port
        if (!z80_n_rd) { //Data port read
            assert_warning((z80_address & 0x00FF) == VDP_PORT_DATA);
            vdp_data_read();
        }
...
///Update VDP internal state.
static void vdp_update() {
